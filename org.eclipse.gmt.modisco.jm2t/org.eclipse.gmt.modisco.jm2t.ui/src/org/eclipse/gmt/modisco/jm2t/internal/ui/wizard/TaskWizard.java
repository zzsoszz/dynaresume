...
import org.eclipse.gmt.modisco.jm2t.internal.ui.wizard.page.WorkspaceRunnableAdapter;
...
public class TaskWizard implements IWizard {
			...
			public void run(IProgressMonitor monitor)
				...
				try {
					...
					while (iterator.hasNext())
						executeTask(iterator.next(), CANCEL,
								monitor);
	...
	public boolean performFinish() {
			...
			public void run(IProgressMonitor monitor) throws CoreException {
				...
				while (index > 0 && index < list.size() - 1) {
					...
					try {
						...
						Trace.trace(Trace.WARNING, "Could not enter/exit page",
								e);
					}
				}
				...
				if (useJob()) {
					class FinishWizardJob extends Job {
						...
						public boolean belongsTo(Object family) {
							return "org.eclipse.wst.server.ui.family"
									.equals(family);
						}

						public IStatus run(IProgressMonitor monitor2) {
							try {
								Iterator<WizardFragment> iterator = list
										.iterator();
								while (iterator.hasNext())
									executeTask(iterator.next(), FINISH,
											monitor2);
							} catch (CoreException ce) {
								Trace.trace(Trace.SEVERE,
										"Error finishing wizard job", ce);
					...
					Iterator<WizardFragment> iterator = list.iterator();
					while (iterator.hasNext())
						executeTask(iterator.next(), FINISH, monitor);
				}
			}
		};
		...
		// TODO: show better error dialog, e.g. when Tomcat config is corrupt
		// while doing Add/Remove
	...
	private void addSubWizardFragments(WizardFragment fragment,
			List<WizardFragment> list) {
		Iterator<WizardFragment> iterator = fragment.getChildFragments()
				.iterator();
		while (iterator.hasNext()) {
	...
	public void addPages() {
		...
		try {
			...
			Iterator<WizardFragment> iterator = getAllWizardFragments()
					.iterator();
			while (iterator.hasNext()) {
